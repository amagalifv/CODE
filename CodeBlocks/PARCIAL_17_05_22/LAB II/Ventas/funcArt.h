...
int buscarPosArt(const char *cod){
    ...
    while (devol==1) {
        devol=fread(&arti ,sizeof(Articulo),1,pArt);

        if (strcmp(arti.getCodigo(),cod)==0 && arti.getEstado()==true) {

            /*if (arti.getEstado()==false) {
                cout<<"Cliente dado de baja";
                ///ACA HAY QUE DECIDIR QUE HACER
            }*/
            return i;
            fclose(pArt);
        }
    ...
    while(true){
        ...
        switch(opc){
            ...
            case 4:
                ...
                break;
            case 5:
                cout<<"Ingrese el dni del codigo del articulo a eliminar: "<<endl;
                cargarCadena(aux, 4);
